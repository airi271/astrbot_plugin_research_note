...
このドキュメントは、Python の基礎はあるがアプリ開発やプラグイン開発は初めて、という前提で書いた学習ロードマップです。目標は AstrBot と Mnemosyne を参考にしながら、根拠付きの「研究補助プラグイン」を最小構成から作ることです。

## 目標

最初の完成形は、特定サービスの再現ではなく、資料に基づく研究補助の中心機能だけを小さく作ることです。
...
根拠付きの回答にするには、質問の前に「関連資料」を追加します。たとえば以下のようなイメージです。
...
研究補助として実用的にするなら、コマンドは1つでは足りません。`/research` の下に複数の操作を作ると分かりやすいです。
...
この状態まで行けば、根拠付き研究補助プラグインの骨格は完成です。
...
この5つに分ければ、研究補助機能も小さい部品として理解できます。
