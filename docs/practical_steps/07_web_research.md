...
```text
/research agent: 保存済み Research Note tools のみ
/research agent_web: 保存済み Research Note tools + allowlist された Web Search tools
```

`agent_web` は `enable_web_research` が `true` のときだけ動きます。`allowed_web_tools` のデフォルトは Tavily 用の `web_search_tavily` と `tavily_extract_web_page` です。
...
"allowed_web_tools": {
  ...
  "type": "list",
  "items": {"type": "string"},
  "default": ["web_search_tavily", "tavily_extract_web_page"]
}
```

`enable_web_research` を default off にする理由です。
...
実装では `agent_web` を採用しています。外部 API を呼ぶ mode が明示されるためです。
