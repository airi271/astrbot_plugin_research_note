...
AstrBot の自由入力を安定して扱うため、短縮 alias も用意しています。

```text
/research import_text <text>
/research import_url <url>
/research import_confirm <pending_id>
```
...
最初は標準ライブラリで HTML を取得します。

追加依存を増やさないため、現行実装は `urllib.request` を `asyncio.to_thread()` で呼び出します。将来、抽出品質を上げる段階で `aiohttp`、Firecrawl、MCP browser などに差し替えます。
...
最初は標準ライブラリの HTML 取得と簡易 text 抽出で十分です。
...
実装では `/research import url <url>` を主コマンドにし、`/research import_url <url>` も短縮 alias として残します。
